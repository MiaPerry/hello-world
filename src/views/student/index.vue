<template>
    <div class="order_deatils_div">
        ...
        <!--添加学员-->
        <div hide-on-blur style="width:100%;height:100%;overflow:hidden">
            <!-- 切换 -->
            ...
            <div class="toInput" :class="{toFocus: focus == true}" v-if="studentIndex==0" style="width: 100%;height:93%;overflow: hidden;position: absolute;left: 0;overflow:scroll">
                <div class="people_ul">
                    <div v-show="form.name.isEcho &&form.name.isEcho==true" :class="{isRequire: form.name.isRequire == true}">
                        <x-input @on-focus="focus = true" @on-blur="focus = false"  :max="8" title="姓 名" placeholder="姓名" :is-type="isName" :required="form.name.isRequire" v-model.trim="form.name.value" label-width="1.2rem" style="padding: 0;"></x-input>
                    </div>
                    <div  v-show="form.sex.isEcho &&form.sex.isEcho==true" style="width: 100%;overflow: hidden" class="color_text color_text1" :class="{isRequire: form.sex.isRequire == true}">
                        <div class="toPad" style="width: 1.2rem;float: left;">性 别</div>
                        <checklist style="width:100%;" label-position="right" :max="1" :required="form.sex.isRequire" :options="sexList" v-model.trim="sex"></checklist>
                    </div>
                    <div v-show="form.idCard.isEcho &&form.idCard.isEcho==true" :class="{isRequire: form.idCard.isRequire == true}">
                        <x-input @on-focus="focus = true" @on-blur="focus = false" :required="form.idCard.isRequire" title="身份证" placeholder="身份证" :is-type="isIDCard" v-model.trim="form.idCard.value" label-width="1.2rem" style="padding: 0;"></x-input>
                    </div>
                    <div v-show="form.mobile.isEcho&&form.mobile.isEcho==true" :class="{isRequire: form.mobile.isRequire == true}">
                        <x-input @on-focus="focus = true" @on-blur="focus = false" :required="form.mobile.isRequire" :disabled="NumDisabled" title="手机号" placeholder="手机号" :is-type="isPhone" v-model.trim="form.mobile.value" label-width="1.2rem" style="padding: 0;"></x-input>
                    </div>
                    ...
                    <div v-if="form.mobile.value.length == 11 && NumDisabled == false ">
                        <x-input @on-focus="focus = true" @on-blur="focus = false" title="验证码" class="weui-vcode" v-model.trim="form.checkCode.value" style="padding: 0;" placeholder="验证码">
                            <x-button slot="right" class="codeBtn" :disabled="disabled" mini @click.native="sendCode">{{codeBtn}}</x-button>
                        </x-input>
                    </div>
                    <div v-show="form.email.isEcho&&   form.email.isEcho==true" :class="{isRequire: form.email.isRequire == true}">
                        <x-input @on-focus="focus = true" @on-blur="focus = false" :required="form.email.isRequire" title="邮箱" placeholder="邮箱" :is-type="isMail" v-model.trim="form.email.value" label-width="1.2rem" style="padding: 0;"></x-input>
                    </div>
                    <div v-show=" form.qq.isEcho&&  form.qq.isEcho==true" :class="{isRequire: form.qq.isRequire == true}">
                        <x-input @on-focus="focus = true" @on-blur="focus = false" :required="form.qq.isRequire" :max="10" title="QQ:" placeholder="QQ" :is-type="isQQ" v-model.trim="form.qq.value" label-width="1.2rem" style="padding: 0;"></x-input>
                    </div>
                    <div v-show="form.year.isEcho&&   form.year.isEcho==true" :class="{isRequire: form.year.isRequire == true}">
                        <selector :required="form.year.isRequire" class="selector" placeholder="考研年份" v-model.trim="form.year.value" title="考研年份" name="district" :options="yearList"></selector>
                    </div>
                    <div v-show="form.grade.isEcho&&   form.grade.isEcho==true" :class="{isRequire: form.grade.isRequire == true}">
                        <selector :required="form.grade.isRequire" class="selector" placeholder="年级" v-model.trim="form.grade.value" title="年级" name="district" :options="gradeList"></selector>
                    </div>
                    <div v-show="form.nowSchool.isEcho&&   form.nowSchool.isEcho==true" :class="{isRequire: form.nowSchool.isRequire == true}">
                        <x-input @on-focus="focus = true" @on-blur="focus = false" :max="20" :required="form.nowSchool.isRequire" title="就读院校" placeholder="就读院校" v-model.trim="form.nowSchool.value" label-width="1.2rem" style="padding: 0;"></x-input>
                    </div>
                    <div v-show="form.nowMajor.isEcho&&   form.nowMajor.isEcho==true" :class="{isRequire: form.nowMajor.isRequire == true}">
                        <x-input @on-focus="focus = true" @on-blur="focus = false" :max="20" :required="form.nowMajor.isRequire" title="就读专业" placeholder="就读专业" v-model.trim="form.nowMajor.value" label-width="1.2rem" style="padding: 0;"></x-input>
                    </div>
                    <div v-show="form.aimSchool.isEcho&&   form.aimSchool.isEcho==true" :class="{isRequire: form.aimSchool.isRequire == true}">
                        <x-input @on-focus="focus = true" @on-blur="focus = false" :max="20" :required="form.aimSchool.isRequire" title="目标院校" placeholder="目标院校" v-model.trim="form.aimSchool.value" label-width="1.2rem" style="padding: 0;"></x-input>
                    </div>
                    <div v-show="form.aimMajor.isEcho&&   form.aimMajor.isEcho==true" :class="{isRequire: form.aimMajor.isRequire == true}">
                        <x-input @on-focus="focus = true" @on-blur="focus = false" :max="20" :required="form.aimMajor.isRequire" title="目标专业" placeholder="目标专业" v-model.trim="form.aimMajor.value" label-width="1.2rem" style="padding: 0;"></x-input>
                    </div>
                    <div v-show="form.source.isEcho&&   form.source.isEcho==true" :class="{isRequire: form.source.isRequire == true}">
                        <selector :required="form.source.isRequire" class="selector" placeholder="来源" v-model.trim="form.source.value" title="来源" name="district" :options="sourceList"></selector>
                    </div>
                    <div v-show="form.remark.isEcho&&   form.remark.isEcho==true" :class="{isRequire: form.remark.isRequire == true}">
                        <x-input @on-focus="focus = true" @on-blur="focus = false" :max="150" :required="form.remark.isRequire" title="备注" placeholder="备注" v-model.trim="form.remark.value" label-width="1.2rem" style="padding: 0;"></x-input>
                    </div>
                    <x-button style="margin-top:0.2rem;background:#3cbaff" @click.native="saveStatue" x-button small type="primary">保存
                    </x-button>
                </div>

            </div>
            <div v-if="studentIndex==1" style="width: 100%;height:1.24rem;overflow:hidden;position: relative;top:0;left: 0;">
                <input class="searchStudent" type="text" v-model="searchStudentName" @on-submit="searchStudent(0)" @keyup.enter="searchStudent(0)" placeholder="学号／姓名／身份证号／手机号">
            ...
            <div v-if="studentIndex==1" style="width: 100%;height:83%;overflow: hidden;position: relative;top:0 ;left: 0;">
                <!-- 滑块 -->
                <scroller lock-x style="position:absolute;top:0;width:100%;height:100%;z-index: 1" ref="scrollerBottom" @on-scroll-bottom="onScrollBottom">
                    <ul class="stu_ul" style="width: 100%;overflow:hidden">
                        <li v-for="(item, index) in searchStudentNameList" :key="index" class="stu_ul_li" style="overflow: hidden;position:relative">
                            <div class="basicInfo">
                                <h3>{{item.name}}</h3>
                                <div  v-show="item.sex != ''" :class="{female: item.sex == 1 }"></div>
                            </div>
                            ...
                            <div class="numInfo">
                                ...
                                <span class="idCard" :class="{onlyOne: item.code == underfine}" v-if="item.idCard">身份证后四位：{{item.idCard}}</span>
                            </div>
...
export default {
    ...
    mounted() {
        ...
        this.selectGradeList(); //年级
        this.selectSourceList(); //代理
        this.selectYearList(); //年份

        this.vuexData = this.$store.getters.getHomeMsg;
        console.log(this.vuexData)
    },
    methods: {
        selectList() {
...
.order_deatils_div .weui-cell_select .weui-select {
    padding-left: 0.2rem;
}
...
@media screen and (min-width: 720px) and (max-width: 799px) {
    ...
    .isRequire{
    background: url("../../styles/img/require.png") no-repeat 0.06rem 0.16rem;
    background-size: 1.7%;
}
}
@media screen and (min-width: 799px) and (max-width: 1024px) {
    ...
    .isRequire{
    background: url("../../styles/img/require.png") no-repeat 0.06rem 0.1rem;
    background-size: 1.7%;
}
...
.basicInfo {
    ...
    div {
        ...
        font-size: 0.2rem;
        color: white;
        background: url("../../styles/img/male.png") no-repeat 0.25rem
            0.035rem #3cbaff;
        background-size: 30%;
        margin-top: 0.19rem;
        padding-left: 0.1rem;
    ...
    .female {
        background: url("../../styles/img/female.png") no-repeat 0.25rem
            0.018rem #fc5860;
